...
目标:把用户上传的 PDF / PPT / HTML / 飞书文档 / 图片 / demo / 素材包识别成结构化输入,拆成“知识层”和“素材层”,让后续 planner 知道讲什么,renderer 知道可用什么,ingestor 知道哪些内容可沉淀。
...
```text
source-dossier.json   # 结构化识别结果
SOURCE_DOSSIER.md     # 给用户/下游 skill 看的摘要
assets/               # 可选:抽取出的图片、缩略图、页面渲染图、附件索引
```
...
本 skill 配有标准库实现的轻量识别器,用于把本地文件/目录/URL 生成 source dossier:

```bash
python3 skills/upload-recognizer/recognize.py \
  path/to/source.pptx path/to/source.html \
  --brief "给零售客户做飞书 Base 提案" \
  --output-dir runs/<task-id>/output
```

它会输出 `source-dossier.json` 和 `SOURCE_DOSSIER.md`。支持:
...
```bash
python3 skills/upload-recognizer/recognize.py path/to/team.pptx \
