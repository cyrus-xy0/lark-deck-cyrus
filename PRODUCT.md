...
| Skill | 责任边界 | 主要产物 |
|---|---|---|
| `upload-recognizer` | 上传识别:把 PDF / PPT / HTML / 飞书文档拆成知识层和素材层 | `source-dossier.json`, `SOURCE_DOSSIER.md` |
...
9. 用户确认入库后,使用已发布的妙笔 deckhtml 作为最终交付入口,再由 recognizer 解析,`deck-ingestor` 将高质量页面、素材和知识分层沉淀:知识/素材优先写 Base,整页 slide 暂留本地候选库;无云端权限时明文回退本地。
...
| 识别层 | 上传物 inventory、知识层/素材层拆解 | `skills/upload-recognizer/` |
...
- 建立上传识别和入库 skill,把 PDF / PPT / HTML / 飞书文档拆成可复用知识层和素材层。
