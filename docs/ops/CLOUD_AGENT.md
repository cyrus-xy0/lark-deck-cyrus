...
7. 使用:agent 收到用户 brief 和素材后调用 `POST /decks`;如有 `sources` / `attachments`,服务端会先跑 parser 并在 run 内创建临时知识/素材库。先把 outline / 状态页发给用户确认;用户确认后调用 `POST /decks/{id}/confirm-outline` 生成 deckhtml、发布到妙笔并生成预演报告。后续用户侧只返回 `magic_url` / `preview_url` 妙笔链接。用户回复要改时调用 `POST /decks/{id}/revise-from-rehearsal` 回到新 outline;用户回复不用改时调用 `POST /decks/{id}/accept-rehearsal` 进入入库确认;确认入库后调用 `POST /decks/{id}/confirm-deck`。
