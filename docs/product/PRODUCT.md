...
5. **交付可闭环**:用户侧只拿妙笔链接;内部每次输出仍保留 deck.json、texts.md、FEEDBACK.md 和素材 manifest。

## 当前设计定稿

`lark-deck-cyrus` 只做全局编排,核心目标是把用户从需求澄清带到一份可以
直接去讲的 pitch deck。具体生产动作拆给六个子 skill:

| Skill | 责任边界 | 主要产物 |
|---|---|---|
| `upload-parser` | 上传解析:把 PDF / PPT / HTML / 飞书文档拆成知识层和素材层 | `source-dossier.json`, `SOURCE_DOSSIER.md` |
...
| `deck-ingestor` | 决定如何沉淀:把通过验收的知识和素材写入 Base,把整页 slide 留在本地候选库 | `ingestion-manifest.json`, `INGESTION_REPORT.md` |

预演反馈不能自动覆盖上一版规划。只有用户确认采纳后,总控才把反馈重新输入
`deck-planner` 和 `deck-renderer` 进入下一轮迭代。

Slide Library 的复用也拆成三层:

- **知识库候选**:服务 `deck-planner`,沉淀“讲什么”,包括场景、主张、证据策略、讲法、风险和客户异议。
- **素材库候选**:服务 `deck-renderer`,沉淀图片、logo、截图、demo、附件和素材来源。
- **Slide 库候选**:服务插页复用,沉淀整页 layout、DeckJSON/HTML 片段、缩略图和标签。

同一页可以进入一层、多层或都不进入;实际写入由 `deck-ingestor` 执行。
当前云端只写知识库和素材库,不建 Slide 云端表。两类记录用 `关联SlideKey`、
`关联素材ID` / `关联知识ID`、来源 deck/PPT 和权限状态互相映射;本地 Slide 库继续
承担整页插入和用户自选 PPT/PPTX 页选择。
...
1. 若用户带 PDF / PPT / HTML / 飞书文档,先由 `upload-parser` 拆成知识层和素材层。
2. `deck-planner` 读取 brief + 知识库 + 解析结果,产出结构化 outline。
3. 用户确认关键判断:目标受众、行业痛点、主张、证据缺口、素材计划。确认前不生成 deckhtml。
4. `deck-renderer` 消费 outline,优先走 DeckJSON-first 生成和渲染。
5. `server/generator.py` 或本地 skill runner 输出 H5、texts.md、FEEDBACK.md、asset manifest,并将 H5 发布为妙笔链接。
...
9. 用户确认入库后,使用已发布的妙笔 deckhtml 作为最终交付入口,再由 parser 解析,`deck-ingestor` 将高质量页面、素材和知识分层沉淀:知识/素材优先写 Base,整页 slide 暂留本地候选库;无云端权限时明文回退本地。
...
3. bot 调用 H5 生成链路,默认只返回妙笔链接和预演报告。
...
| 解析层 | 上传物 inventory、知识层/素材层拆解 | `skills/upload-parser/` |
...
| 复用层 | slide library ingest、locator、反馈闭环 | `skills/deck-ingestor/ingest.py`; `scripts/base_library.py create-*` 只写 Base 知识库/素材库;Slide Library 暂存本地候选库,整页引用通过本地 JSON 保留;支持用户自选 PPT/PPTX 先登记为候选页 |
| 云端部署层 | 用户云端 agent / 飞书 bot 部署契约 | `scripts/cloud_agent_deploy.py`, `server/generator.py serve`, `server/feishu_bot.py serve` |
...
- 建立上传解析和入库 skill,把 PDF / PPT / HTML / 飞书文档拆成可复用知识层和素材层。
