...
| 场景 | 用户说法 | 系统输出 |
|---|---|---|
| 新建客户 pitch | “帮我做一份 XX 客户 AI 知识库 pitchdeck” | 一份可讲的妙笔 deck 链接,内部保留可编辑源 |
...
| `upload-parser` | 上传解析:把 PDF / PPT / HTML / 飞书文档拆成知识层和素材层 | 不规划最终 deck、不渲染、不验收、不入库 | `source-dossier.json`, `SOURCE_DOSSIER.md` |
| `deck-planner` | 讲什么:基于 brief、知识库和解析结果生成每页 message、key idea、emphasis、talk track、proof needed、asset need、layout candidate、risk | 不负责 HTML 细节和最终视觉,不在 planner 后先跑 simulator | `outline.json` |
...
| brief + 其他素材新建或改版 deckhtml | `cyrus -> upload-parser -> 临时知识/素材库 -> deck-planner -> 用户确认 outline -> deck-renderer -> deck-auditor -> pitch-simulator -> 用户确认是否改稿 -> 用户确认是否入库 -> deck-ingestor` | PDF / PPT / HTML / 飞书文档都是素材来源变体,不再拆成独立路线 |
...
| 解析层 | 把上传 PDF / PPT / HTML / 飞书文档拆成知识层和素材层 | `skills/upload-parser/` | source dossier、缩略图、素材候选、provenance 和置信度 |
...
- 输出固定内部产物:`deck.json`、`index.html`、`texts.md`、`FEEDBACK.md`、`assets-manifest.yaml`、可编辑 zip;用户侧交付妙笔链接。
...
- 做 Web 状态页:展示妙笔链接、validator 报告和预演报告。
