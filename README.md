...
- **服务端 wrapper** — `server/generator.py` 固化 `素材识别 -> Outline -> 用户确认 -> DeckJSON -> HTML -> 质量验收/重渲染 -> Pitch 预演 -> 是否改稿 -> 是否入库`
  链路,生成任务目录、运行 renderer / validator,并输出预览、编辑、预演和下载链接字段。
...
- **Deck 入库** — 用户确认入库后,服务端可按配置先把最终 `index.html`
  上传到指定 TOS,再调用 recognizer 解析最终 deckhtml;`skills/deck-ingestor/`
...
|---|---|---|
| 文件大小 | 几十 MB 起步 | 默认内联 HTML 通常 MB 级,编辑包可到几十 MB |
