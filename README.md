...
总控 skill，串联“上传识别 → 临时知识/素材库 → 场景规划 → DeckJSON → HTML 渲染
...
- **服务端 wrapper** — `server/generator.py` 固化 `素材识别 -> Outline -> 用户确认 -> DeckJSON -> HTML -> 质量验收/重渲染 -> 妙笔发布 -> Pitch 预演 -> 是否改稿 -> 是否入库`
...
- **Deck 入库** — 用户确认入库后,服务端使用已发布的妙笔 deckhtml
  作为最终交付入口,再调用 recognizer 解析最终 deckhtml;`skills/deck-ingestor/`
