...
> 浏览器全屏放映、妙笔链接交付、文本侧文件可追溯、视觉与飞书母版完全对齐。
...
```text
skills/       Cyrus 总控和 6 个子 skill 的源码与说明
server/       生成器、飞书 bot、slide library 等产品化 wrapper
scripts/      安装、部署、Base 同步、端到端 pipeline 脚本
evals/        合同测试和产品级 smoke/eval
config/       云端 Base / 素材库配置
knowledge/    随包知识库、行业包和 pitch recipe
library/      本地 slide / 素材 / design-kit 候选库
docs/         产品、运维和开发文档
runs/         本地生成产物，默认被 git 忽略
```
...
- **妙笔链接可直接转发** — 飞书 / 邮件 / IM 里发一个妙笔链接，对方打开即看，不用装 Office
...
更完整的产品方向见 [`docs/product/PRODUCT.md`](docs/product/PRODUCT.md):本项目是 `lark-deck-cyrus`
总控 skill，串联“上传解析 → 临时知识/素材库 → 场景规划 → DeckJSON → HTML 渲染
→ 质量验收 → Pitch 预演 → 用户确认是否改稿 → 用户确认是否入库 → 云端/本地沉淀”的产品闭环。

和 `feishu-deck-h5` 的主要区别: H5 是完整的单体生产 skill; Cyrus 沿用 H5 的
视觉和交付标准,但按场景拆成 parser / planner / renderer / auditor /
...
- **上传解析** — `skills/upload-parser/` 把 PDF / PPT / HTML / 飞书文档
  拆成知识层和素材层。服务端在本轮 run 的 `input/runtime-library/`
  创建临时知识/素材库,再交给 planner、renderer 和 ingestor 使用。
- **知识库** — 默认优先用当前 `lark-cli` user 身份读取云端 `知识库`;无权限或不可达时明文提示并回退随包 `knowledge/`。
- **素材索引** — 默认优先读取云端 `素材库`;无权限或不可达时回退 `assets/shared/asset-index.generated.json`。
...
- **服务端 wrapper** — `server/generator.py` 固化 `素材解析 -> Outline -> 用户确认 -> DeckJSON -> HTML -> 质量验收/重渲染 -> 妙笔发布 -> Pitch 预演 -> 是否改稿 -> 是否入库`
  链路,生成任务目录、运行 renderer / validator,并把用户交付入口收敛为妙笔链接。
...
- **Deck 入库** — 用户确认入库后,服务端使用已发布的妙笔 deckhtml
  作为最终交付入口,再调用 parser 解析最终 deckhtml;`skills/deck-ingestor/`
  把 slide、素材和知识分别写入本地候选库,并默认优先把
...
- **自选 PPT 入库** — 用户可直接上传团队 PPT/PPTX 到 Slide Library 候选库,
  先登记为可搜索/可选择页面,自动生成本地缩略图,再由 parser / renderer
  对选中页拆知识和素材。
- **云端 agent 部署包** — `scripts/cloud_agent_deploy.py` 生成
  ...
  一句话部署方案见 [`docs/ops/CLOUD_AGENT.md`](docs/ops/CLOUD_AGENT.md)。
...
|---|---|---|
| 文件大小 | 几十 MB 起步 | 用户只拿妙笔链接,源文件保留在 run 内用于审计 |
| 需要 Office license | 是 | 否（任何浏览器都能开） |
| 飞书/IM 转发 | 经常变形 | 妙笔链接,对方打开即看 |
...
> "帮我安装 lark-deck-cyrus skill：https://github.com/cyrus-xy0/lark-deck-cyrus，
> 装完帮我做一份关于〔你的主题〕的 deck"

注意:请复制上面的 HTTPS 地址,不要使用 `git@github.com:...` 这种 SSH 地址。
云端 agent 通常没有 GitHub SSH key,看到 SSH 地址会先卡在认证失败。
...
```bash
python3 skills/upload-parser/parse.py path/to/source.pptx \
...
[`docs/ops/CLOUD_AGENT.md`](docs/ops/CLOUD_AGENT.md) 里的“一句话提示词”;部署包也会生成
...
| 内容 | 文档 |
|---|---|
| 安装路径（marketplace / install.sh / 手动 clone） | [docs/ops/INSTALL.md](docs/ops/INSTALL.md) |
| 云端 agent 一句话部署与使用 | [docs/ops/CLOUD_AGENT.md](docs/ops/CLOUD_AGENT.md) |
...
| 9-section 完整设计系统 | [docs/product/DESIGN.md](docs/product/DESIGN.md) |
