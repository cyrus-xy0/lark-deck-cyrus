...
| 只有 brief 新建 deckhtml | `cyrus -> deck-planner -> deck-renderer -> deck-auditor -> pitch-simulator -> deck-ingestor` | planner 基于知识库和 brief 生成 outline,不先跑 simulator |
| brief + 其他素材新建或改版 deckhtml | `cyrus -> upload-recognizer -> deck-planner -> deck-renderer -> deck-auditor -> pitch-simulator -> deck-ingestor` | PDF / PPT / HTML / 飞书文档都是素材来源变体,不再拆成独立路线 |
